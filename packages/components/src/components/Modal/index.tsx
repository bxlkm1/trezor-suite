...
const Heading = styled(H1)<{
    ...
    border-bottom: ${props =>
        props.showHeaderBorder ? `1px solid ${props.theme.NEUE_STROKE_GREY}` : 'none'};
...
const ProgressBarPlaceholder = styled.div<{ hiddenProgressBar: boolean }>`
    ...
    background-color: ${props =>
        props.hiddenProgressBar ? 'transparent' : props.theme.NEUE_STROKE_GREY};
`;

const GreenBar = styled.div<{ width: number }>`
    height: 4px;
    position: relative;
    background-color: ${props => props.theme.NEUE_BG_GREEN};
    transition: all 0.5s;
                ...
                >
                    ...
                    {cancelable && (
                        <CancelIconWrapper data-test="@modal/close-button" onClick={onCancel}>
                            <Icon size={24} color={theme.NEUE_TYPE_DARK_GREY} icon="CROSS" />
                        </CancelIconWrapper>
                    )}
                </Heading>
            )}
