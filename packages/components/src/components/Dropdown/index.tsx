...
const MenuItem = styled.li<MenuItemProps>`
    ...
    color: ${props =>
        !props.item.isDisabled
            ? props.theme.NEUE_TYPE_DARK_GREY
            : props.theme.NEUE_TYPE_LIGHT_GREY};
    ...
    ${props =>
        ...
        css`
            &:hover {
                background: ${props.theme.NEUE_BG_GREY};
            }
        `}
`;
...
const Dropdown = ({
    ...
    const toggleComponent = children ? (
        ...
        <DefaultTogglerIcon
            ...
            size={24}
            icon="MORE"
            color={!isDisabled ? theme.NEUE_TYPE_DARK_GREY : theme.NEUE_TYPE_LIGHT_GREY}
            onClick={
                !isDisabled
                    ? e => {
                          setToggled(!toggled);
    ...
    const getIconComponent = (item: DropdownMenuItem) => {
        if (item.icon) {
            return typeof item.icon === 'string' ? (
                <IconWrapper>
                    <Icon icon={item.icon} size={16} color={theme.NEUE_TYPE_DARK_GREY} />
                </IconWrapper>
            ) : (
                item.icon
            );
