...
const getStatusColor = (status: Status, theme: SuiteThemeColors) => {
    const statusColors = {
        connected: theme.NEUE_TYPE_GREEN,
        disconnected: theme.NEUE_TYPE_RED,
        warning: theme.NEUE_TYPE_ORANGE,
    };
...
const StatusText = styled.div<{ show: boolean; status: Status }>`
    ...
    background: linear-gradient(
        90deg,
        ${props => `${props.theme.NEUE_BG_LIGHT_GREY}00`} 0%,
        ${props => props.theme.NEUE_BG_LIGHT_GREY} 20px,
        ${props => props.theme.NEUE_BG_LIGHT_GREY} 100%
    );
...
const OuterCircle = styled.div<{ show: boolean; status: Status }>`
    ...
    background: ${props =>
        props.status === 'connected'
            ? props.theme.NEUE_BG_LIGHT_GREEN
            : props.theme.NEUE_BG_LIGHT_RED};
    opacity: ${props => (props.show ? 1 : 0)};
