...
const StyledBox = styled.div<{ state?: 'success' }>`
    ...
    ${props =>
        props.state &&
        css`
            border-left: 6px solid ${props => props.theme.NEUE_BG_GREEN};
        `}
    ...
    return (
        <StyledBox state={state === 'success' ? state : undefined}>
            <Row responsive={type === 'regular'}>
                <Left>
                    <IconWrapper>
                        {!state && <Dot color={theme.NEUE_STROKE_GREY} />}
                        {state === 'success' && <Icon color={theme.NEUE_BG_GREEN} icon="CHECK" />}
                        {state === 'warning' && <Dot color={theme.NEUE_TYPE_ORANGE} />}
                    </IconWrapper>
